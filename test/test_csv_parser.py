...
def test_parse_full_row():
    csv_content = (
        "item_url,region,title,content,image_url,category,interest,warehouse,item_name,item_unit_price,item_weight\n"
        "http://a.com,US,Title A,Content A,http://img/a.jpg,1,Tech,WH1,Item A,9.99,1.5"
    )
    result = parse_csv_to_post_data(io.StringIO(csv_content))
    assert len(result) == 1
...
def test_parse_from_file(tmp_path):
    csv_content = (
        "item_url,region,title,content,image_url,category,interest,warehouse,item_name,item_unit_price,item_weight\n"
        "http://b.com,CA,Title B,Content B,http://img/b.jpg,2,Food,WH2,Item B,5.5,0.3"
    )
