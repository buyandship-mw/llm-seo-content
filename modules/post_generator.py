...
def _build_comprehensive_llm_prompt(
    ...
    prompt_lines.append(
        "3. Extract each required data field. If information is unavailable, use these fallbacks: null for image_url, 0.0 for item_price, and 'N/A' for item_currency."
    )
    prompt_lines.append(
    ...
    prompt_lines.append(
        ...
        "Place the numeric price as a float in 'item_price' and the 3-letter currency code in 'item_currency'. "
...
def _finalize_data_from_llm_response(
    ...
    if original_client_input.item_price > 0:
        source_price = original_client_input.item_price
        final_data["item_price"] = source_price
    if original_client_input.item_currency:
        source_currency = original_client_input.item_currency.upper()
        final_data["item_currency"] = source_currency

    llm_price_val = llm_output.get("item_price")
